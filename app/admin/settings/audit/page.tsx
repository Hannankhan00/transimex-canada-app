...
import { AuditLogEntry } from "@/lib/auditTypes";
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            <span className="text-[11px] font-bold uppercase tracking-wider text-[#d21f27]">
              {language === "fr" ? "Responsabilité et Intégrité Réglementaire" : "Regulatory Accountability & Integrity"}
            </span>
            <h1 className="text-3xl sm:text-4xl font-extrabold text-[#0B2545] tracking-tight leading-tight mt-1">
              {language === "fr" ? "Journal d'Audit de l'Activité du Système" : "System Activity Audit Log"}
            </h1>
            <p className="text-slate-500 text-xs sm:text-sm mt-1 max-w-2xl">
              {language === "fr"
                ? "Un registre chronologique permanent consignant les actions réglementaires, les attributions de tarifs et les ajustements de sécurité sur la plateforme."
                : "A permanent chronological ledger recording regulatory actions, rate assignments, and security adjustments across the platform."}
            </p>
          </div>
          ...
          <div className="flex items-center gap-2.5">
            ...
            >
              ...
              <span>{language === "fr" ? "Actualiser le Registre" : "Refresh Ledger"}</span>
            </button>
          ...
          <div className="p-4 sm:p-5 border-b border-slate-200 bg-slate-50/60 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
            <div className="flex flex-wrap items-center gap-2 text-xs">
              ...
              <div>
                ...
                >
                  <option value="all">{language === "fr" ? "Toutes les Catégories d'Action" : "All Action Categories"}</option>
                  {actionsList.map((act) => (
              ...
              <div>
                <select
                  ...
                  className="bg-white border border-slate-200 rounded-xl px-3 py-1.5 text-xs font-semibold text-slate-800 outline-none"
                >
                  <option value="all">{language === "fr" ? "Tous les Acteurs du Personnel" : "All Staff Actors"}</option>
                  {staffList.map((st) => (
            ...
            <div className="relative">
              ...
              <input
                type="text"
                placeholder={language === "fr" ? "Rechercher réf. ressource, personnel, note..." : "Search resource ref, staff, audit note..."}
                value={search}
            ...
            <table className="w-full text-left border-collapse text-xs">
              <thead>
                <tr className="border-b border-slate-200 bg-slate-50/70 text-[11px] font-bold uppercase tracking-wider text-slate-500">
                  ...
                  <th className="py-3.5 px-4">{language === "fr" ? "Membre du Personnel et Rôle" : "Staff Member & Role"}</th>
                  <th className="py-3.5 px-4">{language === "fr" ? "Action Effectuée" : "Action Performed"}</th>
                  <th className="py-3.5 px-4">{language === "fr" ? "Ressource Cible" : "Target Resource"}</th>
                  <th className="py-3.5 px-4">{language === "fr" ? "Détails de l'Audit" : "Audit Details"}</th>
                </tr>
              </thead>
                ...
                {filteredLogs.length === 0 ? (
                  <tr>
                    <td colSpan={5} className="py-12 text-center text-slate-400 text-xs">
                      {language === "fr" ? "Aucun journal d'activité ne correspond aux filtres sélectionnés." : "No activity logs match the selected filters."}
                    </td>
                  </tr>
                ) : (
