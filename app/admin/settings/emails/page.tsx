...
export default function AdminEmailsSettingsPage() {
  ...
  const handleSaveTemplate = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setToastMsg(
        language === "fr"
          ? `Modèle « ${activeTemplate.name} » mis à jour avec succès.`
          : `Template "${activeTemplate.name}" updated successfully.`
      );
  ...
  const handleSendTestEmail = async () => {
    ...
    try {
      ...
      setToastMsg(
        language === "fr"
          ? `Courriel de prévisualisation envoyé à ${testEmailAddress}`
          : `Test preview email dispatched to ${testEmailAddress}`
      );
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            <span className="text-[11px] font-bold uppercase tracking-wider text-[#d21f27]">
              {language === "fr" ? "Points de Contact Client et Envoi Automatisé" : "Customer Touchpoints & Automated Delivery"}
            </span>
            <h1 className="text-3xl sm:text-4xl font-extrabold text-[#0B2545] tracking-tight leading-tight mt-1">
              {language === "fr" ? "Éditeur de Modèles de Courriels Bilingues" : "Bilingual Email Template Editor"}
            </h1>
            <p className="text-slate-500 text-xs sm:text-sm mt-1 max-w-2xl">
              {language === "fr"
                ? "Mettez à jour le contenu anglais et français des courriels opérationnels automatisés sans modification de code ni redéploiement."
          ...
          <div className={`w-full lg:w-80 border-r border-slate-200 bg-slate-50/60 flex flex-col ${mobileView === "editor" ? "hidden lg:flex" : "flex"}`}>
            <div className="p-4 border-b border-slate-200 bg-white">
              <h3 className="font-bold text-xs uppercase tracking-wider text-slate-500">
                {language === "fr" ? "Modèles Opérationnels" : "Operational Templates"}
              </h3>
              <p className="text-[11px] text-slate-400 mt-0.5">
                {language === "fr" ? "Sélectionnez un flux automatisé à personnaliser." : "Select an automated workflow to customize."}
              </p>
            </div>
          ...
          <div className={`flex-1 flex flex-col bg-white ${mobileView === "list" ? "hidden lg:flex" : "flex"}`}>
            {activeTemplate ? (
                ...
                <div className="p-4 sm:p-5 border-b border-slate-200 bg-slate-50/40 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                  <div>
                    ...
                    >
                      ...
                      <span>{language === "fr" ? "Retour aux Modèles" : "Back to Templates"}</span>
                    </button>
                ...
                <div className="p-6 flex-1 space-y-4 overflow-y-auto text-xs">
                  ...
                  <div>
                    <label className="font-bold text-slate-800 block mb-1">
                      {language === "fr" ? "Ligne d'Objet du Courriel" : "Email Subject Line"} ({langTab === "en" ? "English" : "Français"})
                    </label>
                    <input
                      type="text"
                  ...
                  <div>
                    <label className="font-bold text-slate-800 block mb-1">
                      {language === "fr" ? "Titre d'En-tête du Modèle" : "Template Header Title"} ({langTab === "en" ? "English" : "Français"})
                    </label>
                    <input
                      type="text"
                  ...
                  <div>
                    <label className="font-bold text-slate-800 block mb-1">
                      {language === "fr" ? "Contenu du Corps" : "Body Content"} ({langTab === "en" ? "English" : "Français"})
                    </label>
                    <textarea
                      rows={8}
                  ...
                  <div className="p-4 bg-slate-50 rounded-2xl border border-slate-200 space-y-2">
                    <div className="flex items-center justify-between">
                      <span className="font-bold text-slate-700 text-xs flex items-center gap-1.5">
                        ...
                        <span>{language === "fr" ? "Variables Dynamiques à Copier" : "Click-to-Copy Dynamic Variables"}</span>
                      </span>
                      <span className="text-[10px] text-slate-400">
                        {language === "fr" ? "Remplacées dynamiquement lors de l'envoi" : "Replaced dynamically when automated email fires"}
                      </span>
                    </div>
                    ...
                    <div className="flex flex-wrap gap-1.5">
                      {activeTemplate.placeholders.map((token) => (
                        <button
                          ...
                          title={language === "fr" ? "Cliquez pour copier la variable" : "Click to copy variable"}
                        >
                          {copiedToken === token ? (
              ...
              <div className="p-12 text-center text-slate-400 text-xs m-auto">
                {language === "fr" ? "Sélectionnez un modèle à configurer." : "Select a template to configure."}
              </div>
            )}
          </div>
        </div>
